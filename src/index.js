...
console.log("Starting mhfz-documentcore-service service...");
...
const start = async () => {
  process.title = "node-mhfz-documentcore-service";
  ...
  try {
    ...
    await setRedisData("mhfz-documentcore-service:start", today.toUTCString());
    console.log(
      "Service start datetime is written to Redis key mhfz-documentcore-service:start",
      today.toUTCString(),
    );
  } catch (err) {
  ...
  console.log(
    "mhfz-documentcore-service is listening HTTP/REST port " +
...
/* starting the server mhfz-documentcore-service .... */
start();

...
const indexDataToElastic = async (data) => {
  const elasticIndexer = new ElasticIndexer("documentVersion");
  await elasticIndexer.indexData(data);
};

const updateDocumentVersionById = async (id, dataClause) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await DocumentVersion.findOne({
    ...
    const [rowsCount, [dbDoc]] = await DocumentVersion.update(
      dataClause,
      options,
    );
    if (!dbDoc) {
...
module.exports = updateDocumentVersionById;

...
const getDocumentVersionCountByQuery = async (query, stats) => {
  const promises = [];
  const statLabels = [];
  try {
    for (const stat of stats) {
      ...
      if (stat === "count") {
        promises.push(DocumentVersion.count({ where: query }));
        statLabels.push("count");
      } else if (statParts.length == 2) {
        if (statParts[0] === "sum") {
          promises.push(DocumentVersion.sum(statParts[1], { where: query }));
          statLabels.push("sum-" + statParts[1]);
        } else if (statParts[0] === "avg") {
          promises.push(DocumentVersion.avg(statParts[1], { where: query }));
          statLabels.push("avg-" + statParts[1]);
        } else if (statParts[0] === "min") {
          promises.push(DocumentVersion.min(statParts[1], { where: query }));
          statLabels.push("min-" + statParts[1]);
        } else if (statParts[0] === "max") {
          promises.push(DocumentVersion.max(statParts[1], { where: query }));
    ...
    if (promises.length == 0) {
      return await DocumentVersion.count({ where: query });
...
module.exports = getDocumentVersionCountByQuery;

...
const indexDataToElastic = async (data) => {
  const elasticIndexer = new ElasticIndexer("document");
  await elasticIndexer.indexData(data);
};

const updateDocumentById = async (id, dataClause) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const [rowsCount, [dbDoc]] = await Document.update(dataClause, options);
    if (!dbDoc) {

...
describe("updateFileObjectById module", () => {
  let sandbox;
  let updateFileObjectById;
  let FileObjectStub, ElasticIndexerStub;

  const fakeId = "uuid-123";
  const fakeUpdatedData = { id: fakeId, name: "Updated FileObject" };
  ...
  beforeEach(() => {
    ...
    updateFileObjectById = proxyquire(
      "../../../../../src/db-layer/main/FileObject/utils/updateFileObjectById",
      {
  ...
  describe("updateFileObjectById", () => {
    it("should update record by direct ID", async () => {
      const result = await updateFileObjectById(fakeId, { name: "Updated" });
    ...
    it("should extract ID from dataClause if not explicitly provided", async () => {
      const result = await updateFileObjectById(undefined, {
        id: fakeId,
        name: "Updated",
      });
      expect(result).to.deep.equal(fakeUpdatedData);
    ...
    it("should support object-as-ID input", async () => {
      const result = await updateFileObjectById(
        { id: fakeId },
        { name: "Updated" },
      );
      expect(result).to.deep.equal(fakeUpdatedData);
    ...
    it("should throw BadRequestError if no ID is provided", async () => {
      try {
        await updateFileObjectById(undefined, {});
    ...
    it("should throw NotFoundError if no existing record is found", async () => {
      ...
      try {
        await updateFileObjectById(fakeId, { name: "Updated" });
        throw new Error("Expected error");
      } catch (err) {
        expect(err.name).to.equal("HttpServerError");
    ...
    it("should throw HttpServerError if findOne fails", async () => {
      FileObjectStub.findOne.rejects(new Error("DB fail"));
      try {
        await updateFileObjectById(fakeId, { name: "Updated" });
        throw new Error("Expected error");
      } catch (err) {

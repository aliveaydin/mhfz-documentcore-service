...
describe("updateDocumentVersionById module", () => {
  let sandbox;
  let updateDocumentVersionById;
  let DocumentVersionStub, ElasticIndexerStub;

  const fakeId = "uuid-123";
  const fakeUpdatedData = { id: fakeId, name: "Updated DocumentVersion" };
  ...
  beforeEach(() => {
    ...
    updateDocumentVersionById = proxyquire(
      "../../../../../src/db-layer/main/DocumentVersion/utils/updateDocumentVersionById",
      {
  ...
  describe("updateDocumentVersionById", () => {
    it("should update record by direct ID", async () => {
      const result = await updateDocumentVersionById(fakeId, {
        name: "Updated",
      });
    ...
    it("should extract ID from dataClause if not explicitly provided", async () => {
      const result = await updateDocumentVersionById(undefined, {
        id: fakeId,
        name: "Updated",
      });
      expect(result).to.deep.equal(fakeUpdatedData);
    ...
    it("should support object-as-ID input", async () => {
      const result = await updateDocumentVersionById(
        { id: fakeId },
        { name: "Updated" },
      );
      expect(result).to.deep.equal(fakeUpdatedData);
    ...
    it("should throw BadRequestError if no ID is provided", async () => {
      try {
        await updateDocumentVersionById(undefined, {});
    ...
    it("should throw NotFoundError if no existing record is found", async () => {
      ...
      try {
        await updateDocumentVersionById(fakeId, { name: "Updated" });
        throw new Error("Expected error");
      } catch (err) {
        expect(err.name).to.equal("HttpServerError");
    ...
    it("should throw HttpServerError if findOne fails", async () => {
      DocumentVersionStub.findOne.rejects(new Error("DB fail"));
      try {
        await updateDocumentVersionById(fakeId, { name: "Updated" });
        throw new Error("Expected error");
      } catch (err) {

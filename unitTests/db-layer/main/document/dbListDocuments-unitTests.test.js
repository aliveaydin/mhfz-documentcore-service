...
//these tests will work when DbListDocumentsCommand is exported from main code

describe("DbListDocumentsCommand", () => {
  let DbListDocumentsCommand, dbListDocuments;
  ...
  beforeEach(() => {
    ...
    ({ DbListDocumentsCommand, dbListDocuments } = proxyquire(
      "../../../../src/db-layer/main/document/dbListDocuments",
      {
  ...
  describe("constructor", () => {
    it("should initialize command metadata and reset pagination if needed", () => {
      ...
      const cmd = new DbListDocumentsCommand(input);

      expect(cmd.commandName).to.equal("dbListDocuments");
      expect(cmd.emptyResult).to.be.true;
      expect(cmd.objectName).to.equal("documents");
  ...
  describe("getCqrsJoins", () => {
    it("should call Document.getCqrsJoins if defined", async () => {
      const cmd = new DbListDocumentsCommand({});
  ...
  describe("buildIncludes", () => {
    ...
    it("should return [] includes even if getJoins is absent", () => {
      const cmd = new DbListDocumentsCommand({}); // input.getJoins is undefined
  ...
  describe("executeQuery", () => {
    it("should handle empty selectList and skip attributes", async () => {
      const cmd = new DbListDocumentsCommand({ getJoins: false });
      sandbox.stub(cmd, "getSelectList").returns(null);

      await cmd.executeQuery();

      const callArgs = DocumentStub.findAll.getCall(0).args[0];
    ...
    it("should return rows with count if pagination and count query are triggered", async () => {
      DocumentStub.findAll.reset();
      DocumentStub.findAll.onCall(0).resolves([{ id: 1 }, { id: 2 }]);
      DocumentStub.findAll.onCall(1).resolves([{ _COUNT: 12 }]);
      ...
      cmd.executeQuery = async function () {
        const data = await DocumentStub.findAll();
        const count = await DocumentStub.findAll();
        return {
          rows: data,
    ...
    it("should convert aggregations to float or int if convertAggregationsToNumbers exists", async () => {
      ...
      DocumentStub.findAll.reset();
      DocumentStub.findAll.resolves(data);
    ...
    it("should return first element directly if isAggregatedWithoutGroup", async () => {
      ...
      sandbox.stub(cmd, "getSelectList").returns(["total"]);
      sandbox.stub(cmd, "buildIncludes").returns(null);
      DocumentStub.findAll.resolves([first]);
